...
## S3オブジェクトのレプリケーションに失敗した場合にプロセスを再試行するソリューション
- Ｓ３バッチ操作を使用して再試行するＬａｍｂｄａを作成。失敗通知をＬａｍｂｄａに送信するＳ３イベント通知を設定。

## cloudformation と Blue/Greenデプロイを使用。顧客を設定されたパーセンテージではなくランダムにグループを割り当てたい。
- CloudFrontがClookieを設定するようにLambda＠Edgeを使用して、ユーザーをA or　Bに振り分ける

## 北米でアプリホスト。Autrora使っている。ヨーロッパだと遅い。どうするか
- Auroraぐろーばrうデータsベースを使う。ヨーロッパのリージョンにセカンダリクラスターを作成する。

## EC2セキュリティグループとそのインバウンド、アウトバウンドルール確認したい。Lambdaで部門アカウントから取得したい。
- 各部門アカウントにEC2readonlyロール作成。
- 管理アカウントユーザーが部門のローryを受けれるように信頼関係を作成。
- 管理アカウントにロール作成、部門アカウントのロールARNに対してassumeRoleアクションを許可。

## 2つのリージョンにアプリデプロイ。両方のリージョン同時にアクティブ。DBはAuroraGlobal。VPNでリージョン接続。リージョン障害に対応したい。
- レコードセットにレイテンシーに基づくルーティングを設定。ヘルスチェック設定。
- リードレプリカ昇格するLambdaを呼び出す。RDSイベント通知を設定する。

## EC２に作成したユーザーを特定するタグを１時間以内に行う必要あり。
- EC2をイベントソースとして、EventBridgeルールをさk末井。CloudTrailによって配信されたイベントに一致するようにルールを構成します。Lambdaをターゲットにします。
...
## ネットワーク管理のグループ（VPCとか）とアプリ間管理グループ(ELBとか)がある。アプリはネットワーク管理グループのものにデプロイする。考慮事項は。
- VPCへのEc2インスタンスの起動を制限するには、アプリケーショングループのIAMポリシーにリソースレベルの権限が必要。
- EC2インスタンスを含むサブネットを削除しようとすると、ネットワークの更新が失敗します。

## EC2にアプリをデプロイするCFNをさくせいｓ。インスタンスはAmazonLinux。ユーザーデータを含むしぇるを使用しています。ＳＳＭ管理ポリシーがアタッチあされたインスタンスプロファイルがあります。新しいアプリをデプロイするためにユーザーデータを更新。スタックも更新。だが実行中のインスタンスは更新されなかった。
- cfn-init cfn-hup ヘルパースクリプト
- ssm ドキュメントstate manager runcommand

## EKSからSecretManagerのシークレット取得。シークレットはKMSカスタマーマネージドキーで暗号化。　403エラーが返却される。
- カスタマーマネージドキーのポリシーがkubenetesサービスアカウントのIAMロールがキーを使用す露ことを許可していません。
